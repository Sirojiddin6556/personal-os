...
class FinanceService:
    ...
    async def create_account(self, session: AsyncSession, workspace_id: UUID, body: AccountCreate) -> Account:
        acc = Account(
            id=uuid4(),
            ...
            current_balance_minor=body.initial_balance_minor,
            is_archived=False,
        )
        session.add(acc)
    ...
    async def list_accounts(self, session: AsyncSession, workspace_id: UUID) -> List[Account]:
        stmt = select(Account).where(Account.workspace_id == workspace_id, Account.is_archived.is_(False))
        res = await session.execute(stmt)
        return list(res.scalars().all())

    async def create_category(self, session: AsyncSession, workspace_id: UUID, body: CategoryCreate) -> Category:
        cat = Category(
            id=uuid4(),
            workspace_id=workspace_id,
    ...
    async def list_categories(self, session: AsyncSession, workspace_id: UUID) -> List[Category]:
        stmt = select(Category).where(Category.workspace_id == workspace_id).order_by(Category.name.asc())
        res = await session.execute(stmt)
        return list(res.scalars().all())
    ...
    async def post_transaction(
        ...
        stmt = select(Account).where(Account.id == body.account_id, Account.workspace_id == workspace_id).with_for_update()
        res = await session.execute(stmt)
        ...
        if body.type == "transfer":
            stmt_dest = select(Account).where(Account.id == body.destination_account_id, Account.workspace_id == workspace_id).with_for_update()
            res_dest = await session.execute(stmt_dest)
            dest_account = res_dest.scalar_one_or_none()
            if not dest_account:
                raise NotFoundError(resource="Destination Account", identifier=body.destination_account_id)
        ...
        if body.type == "expense":
            source_account.current_balance_minor -= body.amount_minor
        elif body.type == "income":
            source_account.current_balance_minor += body.amount_minor
        elif body.type == "transfer":
            source_account.current_balance_minor -= body.amount_minor
            dest_account.current_balance_minor += body.amount_minor
        ...
        await publish_event(
            session=session,
            event_type="finance.transaction_posted.v1",
    ...
    async def reverse_transaction(
        ...
        stmt = (
            select(Transaction)
            .where(Transaction.id == transaction_id, Transaction.workspace_id == workspace_id)
            .with_for_update()
        )
        res = await session.execute(stmt)
        ...
        # Lock source account
        stmt_acc = select(Account).where(Account.id == orig.account_id).with_for_update()
        ...
        if orig.type == "expense":
            source_account.current_balance_minor += orig.amount_minor
        elif orig.type == "income":
            source_account.current_balance_minor -= orig.amount_minor
        elif orig.type == "transfer":
            stmt_dest = select(Account).where(Account.id == orig.destination_account_id).with_for_update()
            res_dest = await session.execute(stmt_dest)
            dest_acc = res_dest.scalar_one_or_none()
            source_account.current_balance_minor += orig.amount_minor
            dest_acc.current_balance_minor -= orig.amount_minor
        ...
        reversal_tx = Transaction(
            ...
            type="reversal",
            status="posted",
            description=f"Reversal of {orig.id}: {reason or 'Requested reversal'}",
            occurred_at=now,
            posted_at=now,
        )
        ...
        await publish_event(
            session=session,
            event_type="finance.transaction_reversed.v1",
            aggregate_type="transaction",
            aggregate_id=orig.id,
            workspace_id=workspace_id,
            data={"reversed_id": str(orig.id), "reversal_id": str(reversal_tx.id)},
        )
    ...
    async def create_budget(self, session: AsyncSession, workspace_id: UUID, body: BudgetCreate) -> Budget:
        budget = Budget(
            id=uuid4(),
            workspace_id=workspace_id,
            category_id=body.category_id,
