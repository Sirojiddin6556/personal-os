...
- 📋 **Канбан и списки задач:** 8-статусная доменная модель (`inbox`, `todo`, `scheduled`, `in_progress`, `waiting`, `done`, `cancelled`, `archived`) с 5 рабочими колонками Канбан и защитой от конфликтов версий (`If-Match: ETag`).
...
- 💰 **Целостный финансовый учет:** Неизменяемый журнал операций (`posted` transactions immutability), операция сверки остатков (`reconciliation`), сторнирование (`reversal`), целочисленный расчет в минорных единицах (валюта `UZS / сум`), учет категорий и бюджетов.
...
Автоматизированный запуск backend-тестов и проверка типизации фронтенда:
 
```bash
# Backend pytest suite (85 unit & integration tests)
