...
2. **Tasks & Planning:** Задачи, приоритеты, дедлайны, 8-статусная доменная модель (5 рабочих колонок Kanban: `inbox`, `todo`, `scheduled`, `in_progress`, `waiting`; 3 терминальных/скрытых состояния: `done`, `cancelled`, `archived`).
3. **Calendar:** События, тайм-блоки дня, интеграция с внешними календарями.
4. **Finance:** Счета, категории, неизменяемый журнал финансовых операций (`Transaction`) с атомарным расчетом баланса счетов, бюджеты.
